...
class LeRestaurant(MycroftSkill): 
	...
	def handle_restaurant_le(self, message):
		...
		handle_send_message_to_mind_expression()

	def handle_send_message_to_mind_expression(self): query = "สวัสดี" 
		data = {"query": query} response = requests.post(url, 
		headers=headers, json=data) response_data = response.json() 
		templete = 
		response_data['data']['channel-result'][0]['channel-message']['template'] 
		conversation_id = response_data['data']['conversation_id'] 
		self.speak(templete) self.speak(conversation_id)
